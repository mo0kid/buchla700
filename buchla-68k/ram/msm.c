...
int16_t	mpmap[3]   = {MC1_DEV, MC2_DEV, CON_DEV};

/* msmnext[] -- MIDI state machine -- channel message next state table */

int16_t	msmnext[8] = {2, 2, 2, 2, 1, 1, 2, 0};

/* msmcnxt[] -- MIDI state machine -- system message next state table */

int16_t	msmcnxt[8] = {0, 0, 2, 1, 0, 0, 0, 0};

/* ctl2src -- controller to source table */

int16_t	ctl2src[] = {SM_VTMW, SM_LPBR, SM_CTL1, SM_PED1};
...
void msm(void)
{
	register int16_t miport;
	register int16_t midiin;
	register int16_t i;
	register int16_t t1;
	register int16_t t2;
