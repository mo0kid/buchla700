...
int8_t	cmgtags[N_ETYPES] = {	/* score event group sensitivity tags */
...
int8_t	cmgtype[N_ETYPES] = {	/* score event group types for copy / merge */
...
int16_t	ehdlist[N_ETYPES] = {		/* hplist update type table */
...
int16_t oktocm(struct s_entry *ep)
{
	register int16_t et, grp, mode;
...
int16_t oktode(struct s_entry *ep)
{
	register int16_t et, grp, mode;
...
int16_t oktodg(struct s_entry *ep)
{
...
void edelta(struct s_entry *ep, int32_t btime, int32_t delta)
{
	DB_ENTR("edelta");
...
struct s_entry *madjsec(struct s_entry *sbp, int32_t btime)
{
	int16_t ns, nv;
	register int16_t grp, es, et;
	register struct s_entry *ep, *np;
	struct s_entry *cp;
	register int32_t sat;
