...
void hilitpm(int8_t n)
{
	register int16_t chr, h, v, w;
...
void movectc(int16_t r, int16_t c)
{
	ctcpos(r, c);
	aftercm();
}
...
void endpsm(int16_t row, int16_t col)
{
	submenu = FALSE;
	cmtype  = CT_SMTH;
...
void edefstm(int16_t du, int16_t n)
{
	int16_t chan, port, stim;
	register int16_t i, left, right;
...
void ptdest(int16_t du)
{
	register int32_t ltemp;
	register int16_t i, left, right, stim;
	int16_t port, chan;
...
void setsubm(int16_t r, int16_t c)
{
	submenu = TRUE;
	cmtype  = CT_MENU;
	mtcpos(r, c);
...
{
	register int16_t i, box, ptd, ptt;
	register int32_t ltemp;
